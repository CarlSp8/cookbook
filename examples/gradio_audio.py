...
class GeminiHandler(StreamHandler):
    ...
    def _initialize_websocket(self):
        ...
        try:
            ...
            initial_request = {
                "setup": {"model": self.config.model, "tools": [{"google_search": {}}]}
            }
    ...
    with interface:
        gr.HTML(
            ...
            <div style='text-align: center; margin-bottom: 1rem'>
                <h1 style='font-size: 2.5rem; margin-bottom: 0.5rem'>🎙️ Gemini API Voice Chat</h1>
                <p style='font-size: 1.1rem; color: #666'>Real-time voice interaction with Gemini</p>
            </div>
            """
        )
        ...
        if not api_key:
            gr.Markdown(
                ...
                To use this demo, you need to set the `{KEY_NAME}` environment variable.

                1. Get an API key from [Google AI Studio](https://aistudio.google.com/apikey)
                2. Set the environment variable:
                   ```bash
                   export {KEY_NAME}='your_api_key_here'
                   ```
